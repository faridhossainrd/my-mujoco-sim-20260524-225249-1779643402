...
export function setupGUI(parentContext) {
  ...
  // Salute button (Unitree G1 specific)
  controlsFolder.add({salute: () => { parentContext.salute.start(); }}, 'salute').name('Salute');
  document.addEventListener('keydown', (event) => {
    if (event.code === 'KeyS' && !event.ctrlKey) {
      parentContext.salute.start();
      event.preventDefault();
    }
  });
  actionInnerHTML += 'Salute (G1 only)<br>';
  keyInnerHTML += 'S<br>';
